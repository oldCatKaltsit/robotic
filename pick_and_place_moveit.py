...
def point(pose_list):
    a = Pose(Point(pose_list[0],pose_list[1],pose_list[2]-0.93), overhead_orientation)
    return a
...
def main():
    ...
    # 0.74 - 0.93 = -0.15 in Z
    #transformation = tfBuffer.lookup_transform('world','block',rospy.Time())
    ...
    chessPlan = [piece_target_position_map['27'],piece_target_position_map['20'],
    piece_target_position_map['55'],piece_target_position_map['05'],piece_target_position_map['73'],piece_target_position_map['03']]
    pnp.move_to_start(starting_pose)
    #time.sleep(10)
    if not rospy.is_shutdown():
        i = 0
        for key, value in piece_pos_recoder.items():
            init_pos = point(value)
            piece_pos_recoder[key] =chessPlan[i]
            targ_pos = point(chessPlan[i])
            i = i + 1
            pnp.move_to_start(starting_pose)
            print("\nPicking...")
            pnp.pick(init_pos)
